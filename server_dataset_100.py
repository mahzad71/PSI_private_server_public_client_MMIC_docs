...
input_file_name = 'server_dataset.csv'
output_file_base_name = 'server_file_'
summary_file_name = 'server_dataset_NC.csv'
...
try:
    ...
    for i in range(files_needed):
        ...
        current_records_per_file = records_per_file if i < files_needed - 1 else total_records - start_index
    ...
    with open(summary_file_name, 'w', newline='') as summary_file:
        ...
        summary_writer.writerow(['file_number', 'encoded_icd_codes', 'unique_number'])  # Adjusted header to include unique numbers
        ...
        for j in range(files_needed):
            ...
            with open(part_file_name, 'r', newline='') as part_file:
                ...
                for row in reader:
                    # Assuming unique number is the last column
                    summary_writer.writerow([j + 1, row[-3], row[-1]])  # Adjust indices according to actual data layout
